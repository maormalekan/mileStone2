...
void printMap(std::map<std::string, std::vector<float>> map) {
    std::map<std::string,std::vector<float>>::iterator b = map.begin();
    std::map<std::string,std::vector<float>>::iterator e = map.end();
    while (b!=e)
    {
        std::cout<< b->first << ": ";
        for (size_t i = 0; i < b->second.size(); i++) {
            std::cout<<b->second[i]<< std::endl;
        }
        b++;
    }
}
...
int main(){
	...
	TimeSeries ts("/home/maorsdf/advanced-programming-1/ass2/myCsv.txt");
